...
def init_splitter(lang: str):
    ...
    try:
        iso_lang = 'zh' if lang in ['zh-CN', 'zh-TW', 'zh'] else lang
        splitter = SaT("sat-3l", style_or_domain="ud", language=iso_lang)
        # splitter.model.active_adapter = (lang, "ud")
        return splitter
    ...
    def translate_text(index, row, src_lang, dst_lang, splitter):
        ...
        while not success and attempts < max_attempts:
            ...
            try:
                ...
                sentences = deeptranslate(Series(sentences), src_lang, dst_lang, False)
...
def llm_translate(text: str, lang: str, model='qwen2.5:7b', verbose=False):
    # Keep zh-CN for DeepTranslator, but fallback to 'zh' for iso639
    iso_lang = 'zh' if lang in ['zh-CN', 'zh-TW', 'zh'] else lang
    prompt = f"Translate the following text from {Lang(iso_lang)} into English. Do not include empty lines or commentary."
